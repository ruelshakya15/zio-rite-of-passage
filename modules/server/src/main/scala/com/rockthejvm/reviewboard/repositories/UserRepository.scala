...
class UserRepositoryLive private (quill: Quill.Postgres[SnakeCase]) extends UserRepository {
  ...
  inline given insMeta: InsertMeta[User] = insertMeta[User](_.id) // id are generated by db
  ...
  override def update(id: Long, op: User => User): Task[User] =
    for {
      ...
      updated <- run(
        query[User].filter(_.id == lift(id)).updateValue(lift(op(current))).returning(r => r)
      )
    } yield updated
